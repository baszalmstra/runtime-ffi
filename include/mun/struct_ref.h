...
inline std::optional<size_t> find_index(std::string_view type_name,
    ...
    if (it == end) {
        std::cerr << "StructRef `" << type_name << "` does not contain field `"
                  << field_name << "`." << std::endl;
        return std::nullopt;
    }
...
inline std::optional<MunStructInfo> type_info_as_struct(
    ...
    return std::make_optional(
        *reinterpret_cast<const MunStructInfo*>(&type_info + 1));
}
...
class StructRef {
   public:
     ...
     */
    const MunUnsafeTypeInfo info() const noexcept {
        return m_runtime->ptr_type(raw());
    }
    ...
    std::optional<T> get(std::string_view field_name) const noexcept {
        ...
        if (const auto idx =
                details::find_index(type_info->name, struct_info, field_name)) {
            ...
            if (auto diff = reflection::equals_return_type<T>(*field_type)) {
                ...
                std::cerr << "Mismatched types for `"
                          << details::format_struct_field(type_info->name,
                                                          field_name)
            ...
            const auto offset =
                static_cast<size_t>(struct_info.field_offsets[*idx]);
            const auto byte_ptr = reinterpret_cast<const std::byte*>(*raw());
            return std::make_optional(Marshal<T>::copy_from(
                reinterpret_cast<const Marshal<T>::type*>(byte_ptr + offset),
                *m_runtime,
...
struct Marshal<StructRef> {
    ...
    static StructRef copy_from(
        const type* ptr, const Runtime& runtime,
        ...
        if (struct_info.memory_kind == MunStructMemoryKind::Value) {
            ...
            gc_handle = *runtime.gc_alloc(
                const_cast<MunUnsafeTypeInfo>(type_info.value()));
    ...
    static StructRef swap_at(
        type value, type* ptr, const Runtime& runtime,
        std::optional<const MunTypeInfo*> type_info) noexcept {
        // Safety: `type_info_as_struct` is guaranteed to return a value for
        // `StructRef`s.
        const auto struct_info = *type_info_as_struct(*type_info.value());
...
struct ArgumentReflection<StructRef> {
    static const char* type_name(const StructRef& s) noexcept {
        return s.info()->name;
    }
};
