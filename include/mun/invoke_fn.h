...
InvokeResult<Output, Args...> invoke_fn(Runtime& runtime,
                                        std::string_view fn_name,
                                        Args... args) noexcept {
    auto make_error = [](Runtime& runtime, std::string_view fn_name,
                         Args... args) {
    ...
    if (auto fn_info = runtime.find_function_info(fn_name, &error); error) {
        std::cerr << "Failed to retrieve function info due to error: "
                  << error.message() << std::endl;
    } else if (!fn_info) {
        std::cerr << "Failed to obtain function '" << fn_name << "'"
                  << std::endl;
    } else {
        ...
        if constexpr (NUM_ARGS > 0) {
            ...
            const std::optional<std::pair<const char*, const char*>>
                return_type_diffs[] = {
        ...
        if (signature.return_type) {
            ...
            if (auto diff =
                    reflection::equals_return_type<Output>(*return_type)) {
                const auto& [expected, found] = *diff;
            ...
            std::cerr << "Invalid return type. Expected: "
                      << ReturnTypeReflection<void>::type_name() << ". Found: "
                      << ReturnTypeReflection<Output>::type_name() << "."
                      << std::endl;
        ...
        auto fn = reinterpret_cast<typename Marshal<Output>::type (*)(
            typename Marshal<Args>::type...)>(fn_info->fn_ptr);
