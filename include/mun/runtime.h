...
class Runtime {
   ...
   public:
     ...
     */
    Runtime(Runtime&& other) noexcept : m_handle(other.m_handle) {
     ...
     */
    std::optional<MunFunctionInfo> find_function_info(
        std::string_view fn_name, Error* out_error = nullptr) noexcept {
        bool has_fn;
        MunFunctionInfo temp;
        if (auto error = Error(mun_runtime_get_function_info(
                m_handle, fn_name.data(), &has_fn, &temp))) {
            if (out_error) {
    ...
    void gc_root_ptr(MunGcPtr obj) const noexcept {
        assert(mun_gc_root(m_handle, obj)._0 == 0);
    }
...
.*/
inline std::optional<Runtime> make_runtime(
    std::string_view library_path, Error* out_error = nullptr) noexcept {
