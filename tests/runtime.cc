...
TEST_CASE("runtime can be constructed", "[runtime]") {
    ...
    if (auto runtime = mun::make_runtime(
            get_munlib_path("sources/fibonacci.munlib"), &err)) {
        REQUIRE(!err);
    } else {
        REQUIRE(err);
        FAIL(err.message());
...
TEST_CASE("runtime can find `FunctionInfo`", "[runtime]") {
    ...
    if (auto runtime = mun::make_runtime(
        ...
        if (auto function_info =
                runtime->find_function_info("fibonacci", &err)) {
            REQUIRE(!err);
        } else {
            REQUIRE(err);
            FAIL(err.message());
...
TEST_CASE("runtime can garbage collect", "[runtime]") {
    ...
    if (auto runtime = mun::make_runtime(
        ...
        REQUIRE(!err);
        {
            auto res = mun::invoke_fn<mun::StructRef>(*runtime, "new_bool",
                                                      true, false);
            REQUIRE(res.is_ok());
