...
#define TEST_MARSHALLING(ty, lhs, rhs, expected)                            \
    TEST_CASE("function can marshal " #ty, "[marshal]") {                   \
        mun::Error err;                                                     \
        if (auto runtime = mun::make_runtime(                               \
                get_munlib_path("sources/marshal.munlib"), &err)) {         \
            REQUIRE(!err);                                                  \
                                                                            \
            const ty a = (lhs), b = (rhs);                                  \
            auto res = mun::invoke_fn<ty>(*runtime, "marshal_" #ty, a, b);  \
            REQUIRE(res.is_ok());                                           \
            REQUIRE(res.unwrap() == (expected));                            \
        } else {                                                            \
            REQUIRE(err);                                                   \
            FAIL(err.message());                                            \
        }                                                                   \
    ...
    TEST_CASE("struct can get, set, and replace " #ty, "[marshal]") {       \
        ...
        if (auto runtime = mun::make_runtime(                               \
            ...
            const ty a = (lhs), b = (rhs);                                  \
            auto res =                                                      \
                mun::invoke_fn<mun::StructRef>(*runtime, "new_" #ty, a, b); \
            REQUIRE(res.is_ok());                                           \
                                                                            \
            auto s = res.unwrap();                                          \
            {                                                               \
...
TEST_CASE("struct can get, set, and replace struct", "[marshal]") {
    ...
    if (auto runtime = mun::make_runtime(
        ...
        auto gc_wrapper =
            mun::invoke_fn<mun::StructRef>(*runtime, "new_gc_wrapper", a, b);
        REQUIRE(gc_wrapper.is_ok());

        auto value_wrapper =
            mun::invoke_fn<mun::StructRef>(*runtime, "new_value_wrapper", a, b);
        REQUIRE(value_wrapper.is_ok());

        std::array<mun::StructRef, 2> structs = {gc_wrapper.unwrap(),
                                                 value_wrapper.unwrap()};
        for (auto s : structs) {
            // `struct(gc)`
